#!/usr/bin/env bash
# Read-only, fail-closed preflight for all 17 formal V3.2 single-cell cancers.
set -euo pipefail

runtime=./data/CancerLncAtlas/runtime/code_v32_single_cell_ucell_17c_20260827_r2
...
output=./data/CancerLncAtlas/results/model/v32_full_multitask/single_cell_cell_level_17c_preflight_20260827_r2
...
if [[ ! -x "${python}" || ! -f "${runner}" || ! -d "${r6_root}" ]]; then
  echo "Pinned Python, runner, or immutable r6 authority is missing" >&2
  exit 51
fi
if [[ -e "${output}" ]]; then
...
printf '%s\n' \
  ...
  '  "next_gate": "PER_CANCER_SMALL_OFFICIAL_UCELL_PARITY"' \
