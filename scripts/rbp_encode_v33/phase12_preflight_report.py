...
|---|---|---|
""" + "\n".join(
    f"| `{acc}` | PublicationData FileSet | {info.get('total')} (http {info.get('http')}) |"
    for acc, info in (ENCODE.get("plan_accession_audit", {}).get("probe") or {}).items()
) + f"""

The control experiment `{ENCODE.get('plan_accession_audit', {}).get('control_experiment', 'ENCSR720BJU')}`
