#!/usr/bin/env python3
"""Freeze the SARC r3 supervisor bound to the r7 Arrow-schema runner."""
...
TARGET_ROOT = (
    ...
    "single_cell_r7_r7_sarc_gate_20260829_r3"
)
EXPECTED = {
    ...
    "scripts/run_v32_single_cell_r7_r7_sarc_gate.py": (
        2_545,
        "82a5badc5964e8c3d9de4d8b4d8fec119ccc17613dde747b3df8b6b21c682634",
    ),
}
...
def materialize(output_root: Path) -> dict[str, object]:
    ...
    for relative, (expected_bytes, expected_sha) in EXPECTED.items():
        ...
        if (
            ...
            raise SupervisorBridgeError(f"SARC r3 supervisor source drift: {source}")
        entries.append(
            {
                "kind": "file",
    ...
    manifest = {
        ...
        "run_id": "v32-single-cell-r7-r7-sarc-gate-20260829-r3",
    ...
    try:
        ...
        if plan["entry_count"] != 2 or plan["total_bytes"] != 26_285:
            raise SupervisorBridgeError("SARC r3 gate transfer plan drift")
        validation = {
            "format": "CC_HHGT_V3_2_SINGLE_CELL_R7_R7_SARC_GATE_BRIDGE_V1",
            ...
            "audit_root": (
                ...
                "single_cell_r7_r7_sarc_20260829_r3"
            ),
